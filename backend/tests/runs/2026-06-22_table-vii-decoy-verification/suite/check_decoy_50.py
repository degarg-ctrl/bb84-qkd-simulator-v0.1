...
def run_scenario(name, dist, attack_prob, mu):
    ...
    print(f"Norm_signal: {gains['normalized_signal']:.6f}")
    print(f"Norm_decoy:  {gains['normalized_decoy']:.6f}")
    print(f"gain_ratio_diff: {res['gain_difference']:.6f}")
    print(f"Decoy Alert: {res['pns_detected']}")
    print("")

run_scenario("Clean Baseline at 50km", dist=50, attack_prob=0.0, mu=0.2)
